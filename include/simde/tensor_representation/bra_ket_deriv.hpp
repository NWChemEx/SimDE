 ...
 * input fields. For this we use recursive templates. At the end of the recursion
 * we declare the same inputs as for BraKet but then add a field for the first 
 * differentiation variables. Upon return the invoking template adds its
 * differentiation variables, etc. Ultimately, a collection of input fields has
 * been added that includes all differentiation variables.
 *
 ...
 * gradient the dimensionality of the tensor is 1, but we don't know how many atoms
 * the molecular structure has and so we don't know how many coordinates there are 
 * in the gradient).
 */
...
TEMPLATED_PROPERTY_TYPE_INPUTS(BraKetDeriv, N, BraType, OpType, VarDerivN..., VarDeriv, KetType) {
    using op_t = const OpType&;
    using vard_t = const VarDeriv&;
    constexpr char field[] = strcat("Derivative",num_to_string<N>);

    //auto rv = BraKetDeriv<N-1,BraType,OpType,VarDerivN,KetType>(N-1,BraType,OpType,VarDerivN,KetType)
