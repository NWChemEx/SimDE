...
/** @brief The property type for modules that perform Bunch-Kaufman factorizations
 *  of self-adjoint matrices, A.
 *
 ...
 *
 *
 *  self-adjoint rank-2 tensors A(i,j) = conj(A(j,i)). L (U) is a lower (upper) triangular 
 *  rank-2 tensor and D is diagonal.
 *
 *  @tparam MatrixElementType The type of the elements of the input/output tensors.
 */
template<typename MatrixElementType>
struct BunchKaufmanFactorization : public sde::PropertyType<BunchKaufmanFactorization<MatrixElementType>> {
...
auto BunchKaufmanFactorization<MatrixElementType>::inputs_() {
  ...
  rv["Triangle"].set_description("Perform L or U Bunch-Kaufman factorization");
  return rv;
...
auto BunchKaufmanFactorization<MatrixElementType>::results_() {
  ...
  rv["Bunch Kaufman Factor"].set_description("Upper or Lower Triangular Bunch-Kaufman factor");
