 ...
 *
 *  @tparam MatrixElementType The type of the elements of the input tensor and output tensor
 *                      of eigenvectors.
...
>
struct SelfAdjointEigenSolver : public sde::PropertyType<SelfAdjointEigenSolver<MatrixElementType,EigenValueType>> {
    /// Type of the input tensor / output eigenvector tensor that accounts for MatrixElementType
 ...
 *
 *  for self-adjoint rank-2 tensors A(i,j) = conj(A(j,i)), B(i,j) = conj(B(j,i)) with B HPD.
 *  B is referred to as the metric. V is the rank-2 tensor of generalized eigenvectors 
 *  corresponding to the rank-1 tensor of eigenvalues, E.  E will be returned in ascending order.
 *
 *  @tparam MatrixElementType The type of the elements of the input matrix and output tensor
 *                      of eigenvectors.
...
struct GeneralizedSelfAdjointEigenSolver : 
  public sde::PropertyType<GeneralizedSelfAdjointEigenSolver<MatrixElementType,MetricElementType,EigenValueType>> {
    /// Type of the input (matrix) tensor / output eigenvector tensor that accounts for MatrixElementType
    using matrix_tensor_type = type::tensor<MatrixElementType>;
...
template<typename MatrixElementType, typename MetricElementType, typename EigenValueType>
auto GeneralizedSelfAdjointEigenSolver<MatrixElementType, MetricElementType, EigenValueType>::inputs_() {
    auto rv = sde::declare_input().
                add_field<matrix_tensor_type>("Input Matrix").
                template add_field<metric_tensor_type>("Input Metric");

    rv["Input Matrix"].set_description("A of the (A,B) matrix pencil to be diagonalized");
    rv["Input Metric"].set_description("B of the (A,B) matrix pencil to be diagonalized");
    return rv;
...
auto SelfAdjointEigenSolver<MatrixElementType, EigenValueType>::results_() {
    ...
    rv["Eigenvalues"] .set_description("The vector of eigenvalues for the input matrix");
    rv["Eigenvectors"].set_description("The matrix of eigenvectors for the input matrix");
    return rv;
}; // SelfAdjointEigenSolver<MatrixElementType,EigenValueType>::results_

template<typename MatrixElementType, typename MetricElementType, typename EigenValueType>
auto GeneralizedSelfAdjointEigenSolver<MatrixElementType, MetricElementType, EigenValueType>::results_() {
    auto rv = sde::declare_result().
                add_field<matrix_tensor_type>("Eigenvectors").
                template add_field<eigenvalue_matrix_tensor_type>("Eigenvalues");

    rv["Eigenvalues"] .set_description("The vector of eigenvalues for the input matrix pencil");
    rv["Eigenvectors"].set_description("The matrix of eigenvectors for the input matrix pencil");
    return rv;
...
extern template class GeneralizedSelfAdjointEigenSolver<std::complex<double>,std::complex<double>,double>;
extern template class GeneralizedSelfAdjointEigenSolver<std::complex<float>,std::complex<float>,float>;
extern template class GeneralizedSelfAdjointEigenSolver<std::complex<double>,double,double>;
extern template class GeneralizedSelfAdjointEigenSolver<std::complex<float>,float,float>;
